...
const Dashboard = () => {
  ...
  const fetchTasks = async () => {
    setLoading(true);
    try {
      const res = await axios.get(`http://localhost:5000/api/tasks?status=${filter}&search=${search}`);
      setTasks(res.data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (currentTask) {
        const res = await axios.put(`http://localhost:5000/api/tasks/${currentTask._id}`, formData);
